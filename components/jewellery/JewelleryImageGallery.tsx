...
export function JewelleryImageGallery({ images = [], itemName, locale = "en" }: JewelleryImageGalleryProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        ...
        className="h-64 sm:h-80 md:h-96 rounded-2xl sm:rounded-3xl bg-earth-950 border border-gold-500/30 overflow-hidden shadow-2xl relative cursor-pointer group select-none touch-pan-y"
      >
        {/* Ambient Blur Layer */}
        ...
        <img
          src={activeImage.image_url}
          alt={activeImage.alt_text?.replace("[hidden]", "").trim() || itemName}
        ...
        <div className="absolute inset-0 z-20 bg-gradient-to-t from-earth-950/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity p-4 flex items-end justify-between text-white pointer-events-none">
        ...
        {sortedImages.length > 1 && (
          <span className="absolute bottom-3 right-3 z-20 px-3 py-1 rounded-full bg-earth-950/85 text-cream-100 text-xs font-semibold backdrop-blur-md shadow-md border border-gold-500/20">
        ...
        {sortedImages.length > 1 && (
          <>
            <button
              ...
              className="absolute left-2 top-1/2 -translate-y-1/2 z-20 p-2 rounded-full bg-earth-950/70 border border-gold-500/30 text-gold-300 hover:text-white hover:bg-earth-900 transition-all opacity-0 group-hover:opacity-100 hidden sm:flex items-center justify-center shadow-lg"
            ...
            <button
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 z-20 p-2 rounded-full bg-earth-950/70 border border-gold-500/30 text-gold-300 hover:text-white hover:bg-earth-900 transition-all opacity-0 group-hover:opacity-100 hidden sm:flex items-center justify-center shadow-lg"
          ...
          <div className="flex items-center gap-2.5 overflow-x-auto pb-1 scrollbar-none">
            {sortedImages.map((img, idx) => (
              <button
                ...
                className={`relative w-20 h-20 sm:w-24 sm:h-24 rounded-xl overflow-hidden border-2 shrink-0 transition-all bg-earth-950 ${
                  activeIdx === idx
